...
class ModelFactory {
    ...
    public function __get($name) {
        if (!is_array($this->models) || $this->models == null){
            return null;
        }
    ...
    private function create($model) {
        ...
        if ($reqFile == null){
            RunException::throwException("找不到模型类文件 $modelFile.php");
        }
        require_once($reqFile);
        if (!class_exists($model)){
            RunException::throwException('找不到Model类 ' . $model);
        }
        $objModel = new $model();
        //把组件容器传递给模型
    ...
    private function findFile($model) {
        ...
        if (!file_exists($this->path)){
            RunException::throwException('找不到Model目录 ' . $this->path);
        }
