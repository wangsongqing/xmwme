...
class Wechat
{
	...
	const MSGTYPE_LOCATION = 'location';//上报地理位置
	const MSGTYPE_LINK = 'link';
	const MSGTYPE_EVENT = 'event';//事件推送
	const MSGTYPE_MUSIC = 'music';//音乐消息
	const MSGTYPE_NEWS = 'news';//图文消息
	const MSGTYPE_VOICE = 'voice';//语音消息
	const MSGTYPE_VIDEO = 'video';//视频消息//shortvideo
	...
	public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $postStr = preg_replace('/[\x00-\x08\x0b-\x0c\x0e-\x1f\x7f]/', '', $postStr);
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		$access_token = getVar($appid.'access_token');
		if($access_token){//die($access_token);
    		...
    		if ($result)
    		{
    			...
    			setVar($appid.'access_token',$this->access_token,$expire);
	...
	public function getJsTicket($appid='',$jsapi_ticket=''){
        ...
        $jsapi_ticket = getVar('jsapi_ticket'.$appid);
        if($jsapi_ticket){
            if(time()>strtotime('2015-06-28 00:00:00')){
                $count2 = getVar('jscount2') ? getVar('jscount2') : 0;
                $count2++;
                setVar('jscount2',$count2,86400);
            }
            $this->jsapi_ticket = $jsapi_ticket;
		...
		if ($result)
		{
			...
			$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
            if(time()>strtotime('2015-06-28 00:00:00')){
                $count = getVar('jscount') ? getVar('jscount') : 0;
                $count++;
                setVar('jscount',$count,86400);
            }
            setVar('jsapi_ticket'.$appid,$this->jsapi_ticket,$expire);
	 ...
	 * @return raw data
	 */
	public function saveMedia($media_id,$dirname){
	    if (!$this->access_token && !$this->checkAuth()) return false;
	 ...
	 */
	public function getQRCode($scene_str,$type=0,$expire=1800){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
			'action_name'=>$type?"QR_LIMIT_STR_SCENE":"QR_SCENE",
	...
	public function getUserBatchInfo($data){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.self::USER_BATCH_INFO_URL.'access_token='.$this->access_token,self::json_encode($data));
    ...
    public function test(){
        setVar('wechat_test','hello');
        echo getVar('wechat_test');
    }
